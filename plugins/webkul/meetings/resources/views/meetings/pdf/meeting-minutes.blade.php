...
<body>
    <div class="header">
        ...
        <div>
            ...
            <div>{{ __('meetings::meetings.fields.meeting_date') }}: {{ $meeting->meeting_date?->format('Y-m-d H:i') }}</div>
        </div>
    </div>
    ...
    <table>
        ...
        <tbody>
            @foreach ($meeting->approvals->flatMap->actions as $approvalAction)
                <tr>
                    <td>{{ $approvalAction->type?->value ?? $approvalAction->type }}</td>
                    <td>{{ $approvalAction->user?->name ?: '-' }}</td>
                    <td>{{ $approvalAction->created_at?->format('Y-m-d H:i') }}</td>
    ...
    <div class="footer">
        {{ __('meetings::meetings.pdf.footer', ['number' => $meeting->meeting_number, 'date' => now()->toDateString()]) }}
    </div>
</body>
</html>
